...
use App\Http\Requests\SupplierReference\ImportRequest;
use App\Http\Requests\SupplierReference\StoreRequest;
...
class SupplierReferenceController extends Controller
{
    ...
    public function index(StatusRequest $request)
    {
        ...
        $supplier_type = SupplierReference::when(
            $status === "inactive",
            function ($query) {
    ...
    {
        $supplier_type = SupplierReference::find($id);
        if (!$supplier_type) {
            return $this->responseNotFound("Nothing to display.");
        }
    ...
    {
        $supplier_type = SupplierReference::create([
            "name" => $request->name,
        ]);
        ...
        //     "user_id" => $user_login,
        //     "action" => "Create",
        //     "module" => "SupplierReference Module",
    ...
    {
        $supplier_type = SupplierReference::where("id", $id)
            ->withTrashed()
            ->get();
        ...
        $supplier_type = SupplierReference::withTrashed()->find($id);
        $is_active = SupplierReference::withTrashed()
            ->where("id", $id)
            ->first();
        if (!$is_active) {
            return $is_active;
    ...
    {
        $import = $request->all();
        $business_unit = SupplierReference::upsert($import, ["name"]);
